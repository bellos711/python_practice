...
def stats_page(request):
    print("#"*50)
    print("Entering stats page")
    this_user = User.objects.get(id=request.session['uuid'])
    this_user_wishes = this_user.wish_uploaded.all()
    context = {
        'user_object': this_user,
        'all_users_wishes' : this_user_wishes.filter(is_granted=False),
        'all_user_wishes_granted' : this_user_wishes.filter(is_granted=True),
        'all_wishes_granted' : Wish.objects.filter(is_granted=True)
    }
...
def remove_wish_logic(request, wish_id):
    this_wish = Wish.objects.get(id=wish_id)
    print("#"*30)
    print("You are REMOVING this wish ", this_wish.title)
    this_wish.delete()
    return redirect('/wishes')